...
__export(src_exports, {
  ...
  MediaType: () => MediaType,
  akaSwapUser: () => akaSwapUser,
  getIpfsSrc: () => getIpfsSrc,
...
var import_fetch_node_details = __toESM(require("@toruslabs/fetch-node-details"));
var import_torus = __toESM(require("@toruslabs/torus.js"));
...
function pk2pkh(pk) {
  if (pk.length === 54 && pk.slice(0, 4) === "edpk") {
    ...
    return b58cencode((0, import_blakejs.blake2b)(pkDecoded, void 0, 20), prefix.tz1);
  } else if (pk.length === 55 && pk.slice(0, 4) === "sppk") {
    const pkDecoded = b58cdecode(pk, prefix.edpk);
    return b58cencode((0, import_blakejs.blake2b)(pkDecoded, void 0, 20), prefix.tz2);
  }
  throw new Error("Invalid public key");
}
function b58cdecode(enc, prefixx) {
  let n = import_bs58check.default.decode(enc);
...
function b58cencode(payload, prefixx) {
  ...
  return import_bs58check.default.encode(Buffer.from(buf2hex(n), "hex"));
}
...
async function lookupPkh(selectedVerifier, verifierId) {
  ...
  const fetchNodeDetails = new import_fetch_node_details.default({ network: "mainnet" });
  const torus = new import_torus.default({
